...
const RevenueScreen = () => {
  ...
  const [selectedLocation, setSelectedLocation] = useState<any>(courtItem || null);
  ...
  const renderBookingItem = ({ item }: { item: BookingData }) => (
    <View style={styles.card}>
      <View style={styles.row}>
        <Ionicons name="person-outline" size={18} color="black" style={styles.icon} />
        <View>
          <Text style={styles.cardTextBold}>{item.playerName}</Text>
          <Text style={{ fontSize: 12, color: '#666' }}>{item.playerPhone}</Text>
        </View>
      </View>
      <View style={styles.row}>
        <Ionicons name="tennisball-outline" size={18} color="#3B9AFF" style={styles.icon} />
        <Text style={styles.cardText}>Sân: {item.courtName}</Text>
      </View>
      <View style={styles.row}>
        <Ionicons name="time-outline" size={18} color="#e67e22" style={styles.icon} />
        <Text style={styles.cardText}>
          {item.bookingDate} | {formatTime(item.startHours)}
        </Text>
      </View>
      <View style={styles.row}>
        <Ionicons name="cash-outline" size={18} color="black" style={styles.icon} />
        <Text style={[styles.cardText, { color: '#e74c3c', fontWeight: 'bold' }]}>
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.blueHeader}>
        <Text style={styles.headerText}>Quản lý doanh thu hiệu quả - Chính xác!</Text>
      </View>
      ...
      <View style={styles.body}>
        ...
        <View style={styles.titleContainer}>
          ...
          <TouchableOpacity style={styles.locationSelector} onPress={() => setModalVisible(true)}>
            <Text style={styles.courtTitle}>
              {selectedLocation?.name || "Chọn cơ sở"}
            </Text>
            <Ionicons name="caret-down-circle" size={20} color="#3B9AFF" style={{marginTop: 5, marginLeft: 5}}/>
          </TouchableOpacity>
        </View>
        ...
        <View style={styles.filterBar}>
          ...
          >
            ...
            <Text style={{ marginLeft: 10, color: "#333", flex: 1, fontWeight: '500' }}>
        ...
        */}
            ...
            <View style={Platform.OS === 'ios' ? styles.iosDatePickerContainer : undefined}>
                ...
                {Platform.OS === 'ios' && (
                    <TouchableOpacity 
                        ...
                        style={{padding: 10, borderTopWidth: 1, borderColor: '#eee', width: '100%', alignItems: 'center'}}
                    >
                        <Text style={{color: '#3B9AFF', fontWeight: 'bold'}}>Đóng</Text>
                    </TouchableOpacity>
                )}
            </View>
        )}
        ...
        {loading ? (
          <View style={styles.loadingContainer}>
            ...
            <Text style={styles.loadingText}>Đang tải dữ liệu {selectedLocation?.name}...</Text>
          </View>
        ) : (
          <FlatList
            data={bookings}
      ...
      <View style={styles.footer}>
        <View style={styles.footerRow}>
          ...
          <Text style={styles.footerAmount}>{formatCurrency(totalRevenue)}</Text>
        </View>
        <View style={styles.footerRow}>
          <Text style={styles.footerDate}>{selectedLocation?.name}</Text>
          <Text style={styles.footerIncrease}>{bookings.length} lượt khách</Text>
        </View>
      </View>
          ...
          keyExtractor={(item, index) => item?.id ? item.id.toString() : index.toString()}
              renderItem={({ item }) => (
                <TouchableOpacity
                  style={[
                    styles.modalItem,
                    selectedLocation?.id === item.id && styles.modalItemSelected
                  ]}
                ...
                >
                  <Ionicons 
                    name="business" 
                    size={20} 
                    color={selectedLocation?.id === item.id ? "#3B9AFF" : "#555"} 
                  />
                  <Text style={[
                    styles.modalItemText,
                    selectedLocation?.id === item.id && styles.modalItemTextSelected
                  ]}>
                    {item.name}
                  </Text>
                  {selectedLocation?.id === item.id && (
                    <Ionicons name="checkmark-circle" size={20} color="#3B9AFF" style={{marginLeft: 'auto'}}/>
                  )}
                </TouchableOpacity>
              )}
            />
