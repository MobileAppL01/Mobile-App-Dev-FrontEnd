...
export const bookingService = {
    ...
    holdSlot: async (request: HoldSlotRequest): Promise<HoldSlotResponse> => {
        const response = await axiosInstance.post('/enhanced-booking-payment/step1-hold', request);
        return response.data;
    },

    // Step 2: Process Payment
    processPayment: async (request: ProcessPaymentRequest): Promise<ProcessPaymentResponse> => {
        const response = await axiosInstance.post('/enhanced-booking-payment/step2-payment', request);
        return response.data;
    },

    // Get Payment Info
    getPaymentInfo: async (bookingId: number): Promise<PaymentInfoResponse> => {
        const response = await axiosInstance.get(`/enhanced-booking-payment/${bookingId}/payment-info`);
        return response.data;
    },

    // Check Status
    checkPaymentStatus: async (paymentId: number) => {
        const response = await axiosInstance.get(`/enhanced-booking-payment/${paymentId}/status`);
        return response.data;
    },

    // Available Payment Methods
    getAvailablePaymentMethods: async () => {
        const response = await axiosInstance.get('/enhanced-booking-payment/available-payment-methods');
        return response.data;
    },
    ...
    getOwnerBookings: async (params?: { locationId?: number, date?: string, status?: string }) => {
        try {
